...
if (isset($_GET['del'])) {
  $id = (int)$_GET['del'];
  $st = $conn->prepare("DELETE FROM income WHERE income_id=? AND user_id=?");
...
/* ดึงรายการ */
$st = $conn->prepare("
  SELECT i.income_id, i.occurred_at, i.amount, i.note,
         c.name AS category_name, pm.method_name
  FROM income i
  LEFT JOIN categories c ON i.category_id = c.category_id
  LEFT JOIN paymentmethods pm ON i.method_id = pm.method_id
  WHERE i.user_id=? AND i.occurred_at BETWEEN ? AND ?
  ORDER BY i.occurred_at DESC, i.income_id DESC
");
...
<style>
  ...
  h2{color:#16a34a;margin-bottom:16px;font-weight:600;}
  .toolbar{display:flex;gap:10px;align-items:center;margin-bottom:20px;flex-wrap:wrap;}
  input[type=month]{padding:8px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:14px;}
  .btn{background:#16a34a;color:#fff;border:none;padding:8px 16px;border-radius:8px;cursor:pointer;transition:0.2s;}
  ...
  .amount{color:#16a34a;font-weight:500;text-align:right;}
  ...
  <table>
    ...
    <?php foreach($rows as $r): ?>
      <tr>
        ...
        <td><?=esc($r['note'] ?? '')?></td>
        <td><a class="danger" href="?del=<?=$r['income_id']?>" onclick="return confirm('ลบถาวรรายการนี้?')">ลบ</a></td>
      </tr>
